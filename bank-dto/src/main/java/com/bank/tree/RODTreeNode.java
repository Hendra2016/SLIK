 ...
 * 
 * The TreeNode that supports ROD, with any data bean that implements interface
 * RODTreeNodeData<br>
 * <br>
 * 
 * The basic rules:<br>
 * 
 * 1. Only call getChildren while you really want to get children or the
 * children has been loaded<br>
...
public class RODTreeNode<T extends RODTreeNodeData> {
	...
	public RODTreeNode(RODTreeNode<T> root) {
		this._parent = root;
	}
	...
	public List<RODTreeNode<T>> getChildren() {
		if (_children == null) {
			...
			for (T data : childrenData) {
				RODTreeNode<T> child = new RODTreeNode<T>(data,
						(List<RODTreeNode<T>>) null);
				child.setParent(this);
				_children.add(child);
			}
		}
	...
	/**
	 * get child count from children size if children created, or get child
	 * count from data
	 * 
	...
	public int getChildCount() {
		int count = _children == null ? (_data == null ? 0 : _data
				.getChildCount()) : _children.size();
